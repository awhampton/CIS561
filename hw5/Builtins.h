 ...
 * The receiver object ('this_in' in Quack) is an implicit argument
...
typedef struct obj_String_struct {
  class_String clazz;
  char *text;     /* FIXME: Make this_in a garbage-collectable rope */
} * obj_String;
 ...
 * this_in is not available to the Quack programmer, but
...
obj_String Int_method_STRING(obj_Int this_in);
